...
import { initialListQuestions } from "./editAhorcado";
import { hangStyles } from "./editAhorcado";
...
export default function Ahorcado({ enviarDatos, enviarDatos2, enviarDatos3, i }) {
  const [listaPreguntas, setListasPreguntas] = useLocalStorage(
    "objectAhorcado",
    initialListQuestions
  );
  const [indexPregunta, setIndexPregunta] = useState(1); //Estado: Index para pasar de pregunta
  ...
  const [error, setError] = useState(0); //Estado: Contador de errores
  const [buttonActive, setButtonActive] = useState(false); //Estado: Boolean para boton de siguiente pregunta
  const [showCorrect, setShowCorrect] = useState(false); //Estado: Mostrar icono de correcto
  const [showIncorrect, setShowIncorrect] = useState(false); //Estado: Mostrar icono de incorrecto

  const [flags, setFlags] = useLocalStorage("arrayFlags", ""); //Estado; Array de flags de palabras terminadas
  const [counterAhorcado, setCounterAhorcado] = useState(0); //Estado: Contador de clics
  ...
  const renderPreguntas = listaPreguntas.map(
    (pregunta) =>
      pregunta.active && (
  ...
  const ErrorRender = ({ count }) => {
    //Se le pasa contador de errores
    const renderImgXMark = () => {
      const paragraphs = [];

      for (let i = 0; i < count; i++) {
        //Dependiendo de eso genera una cantidad de taches
        paragraphs.push(
          <img
            key={i}
            className="imgXError"
  ...
  const siguientePregunta = (i) => {
    const siguienteListaPregunta = listaPreguntas.map((pregunta) => {
      if (i - 1 == pregunta.id) {
        //La pregunta anterior se desactiva
        return {
          ...pregunta,
          active: false,
        };
      } else if (i == pregunta.id) {
        //La pregunta nueva se muestra
        return {
          ...pregunta,
          active: true,
        };
  ...
  return (
    <div className="contenedorPrincipal">
      ...
      <p className="contadorAvance">
        {flags.length} / {listaPreguntas.length}
      </p>
    </div>
  );
}

...
import { initialListCardZoom } from "./EditCardZoom";
import { CardZoomStyles } from "./EditCardZoom"; 

export default function CardZoom({ enviarDatos, enviarDatos2, enviarDatos3, i }) {
  const [listCardZoom, setListCardZoom] = useLocalStorage(
    "objectCardZoom",
    initialListCardZoom
  );
  const [flags, setFlags] = useLocalStorage("flagsHoverDos", "");
  const [counterCardZoom, setCounterCardZoom] = useState(0);

  const RenderCardZoom =  ({ card }) => {
